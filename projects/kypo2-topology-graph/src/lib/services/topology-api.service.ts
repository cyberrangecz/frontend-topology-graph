...
export class TopologyApi {
   ...
   */
  establishGuacamoleRemoteConnection(sandboxId: number, vmName: string, vmIp: string, userInterface: UserInterface): Observable<string> {
    return zip(this.getGuacamoleToken(), this.getVmInfo(sandboxId, vmName), this.getManIp(sandboxId))
      .pipe(
        switchMap(resp => {
          ...
          return this.createGuacamoleQuickConnection(resp[0].authToken, vmIp, resp[1].image.os_type, resp[2].ip, userInterface);
        }),
        tap(
          {
            error: err =>  {
  ...
  private getGuacamoleToken() {
    ...
    return this.http.post<GuacamoleTokenDTO>(`${this.configService.config.guacamoleConfig.url}api/tokens`, body.toString(), {
      headers: new HttpHeaders({'Content-Type':  ' application/x-www-form-urlencoded'})})
  }

  private getVmInfo(sandboxId: number, vmName: string): Observable<HostDTO> {
    return this.http.get<HostDTO>(`${this.configService.config.topologyRestUrl}sandboxes/${sandboxId}/vms/${vmName}`)
      .pipe(
        tap(
          {
            error: err =>  {
  ...
  private getManIp(sandboxId: number): Observable<ManDTO> {
    return this.http.get<ManDTO>(`${this.configService.config.topologyRestUrl}sandboxes/${sandboxId}/man-out-port-ip`)
      .pipe(
        tap(
          {
            error: err =>  {
  ...
  private getQuickConnectURI(vmIp: string, vmOsType: string, manIp: string, userInterface: UserInterface): Observable<URLSearchParams> {
    const body = new URLSearchParams();
  ...
  private createGuacamoleQuickConnection(token: string, vmIp: string, vmOsType: string, manIp: string, userInterface: UserInterface) {
    return this.getQuickConnectURI(vmIp, vmOsType, manIp, userInterface).pipe(
      switchMap(urlSearchParams => {
        // eslint-disable-next-line max-len
          return this.http.post<GuacamoleIdentifierDTO>(`${this.configService.config.guacamoleConfig.url}api/session/ext/quickconnect/create`,
            urlSearchParams.toString(), {
            headers: new HttpHeaders({'Content-Type': ' application/x-www-form-urlencoded'}),
            params: new HttpParams().set('token', token)
          }).pipe(
            map(resp => resp.identifier));
      }),
      tap(
        {
          error: err =>  {
